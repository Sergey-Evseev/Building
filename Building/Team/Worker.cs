    ...
    internal class Worker : IWorker
    {
        ...
        public void Build(House house, TeamLeader leader) 
        {
            if (house.basement == null)
            {
                ...
                leader.progressList.Add($"Worker {Name} built the basement");
            }
            else if (house.walls == null || house.walls.Count < 4)
            {
                ...
                wall.create(house);
                leader.progressList.Add($"Worker {Name} built the wall #{house.walls.Count}");
            }
            else if (house.door == null)
            {
                Door door = new Door();
                door.create(house);
                leader.progressList.Add($"Worker {Name} installed the door");
            }
            ...
            {   //добавление окон в список окон
                ...
                window.create(house);
                leader.progressList.Add($"Worker {Name} installed the window #{house.window.Count}");
            }
            else if (house.roof == null)
            {
                Roof roof = new Roof();
                roof.create(house);
                leader.progressList.Add($"Worker {Name} built the roof");
            }        
        }
    }
}
